...
class Default_HoteisController extends CORE_Controller_Action
{
    protected $_dbVersaoBanco;

    public function init()
    {
        parent::init();
        if (null === $this->_db) {
            $this->_db = new Model_Hotel();
        }

        $this->_form = new Form_Hotel();
        
        
        $this->_redirecionar = "/hoteis";
        $this->view->controller = "Hoteis"; 
    }
    ...
    public function editAction() 
    {
        ...
        if ($this->_request->isPost()) {
            $this->_form->getElement('nome')->removeValidator('Zend_Validate_Db_NoRecordExists');
            $this->_data = $this->_request->getPost();
            if ($this->_form->isValid($this->_data)) {
                $data = $this->_form->getValues();
